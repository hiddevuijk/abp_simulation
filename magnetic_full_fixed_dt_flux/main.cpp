...
int main(int argc, char *argv[])
{
	...
	int navg;
	int N, seed;
	double Dt,Dr,v0,m,qB,omega0,beta,eps,sigma,L,dt,tf,teq;
	double rho;
	...
	read_variables_magnetic(N,rho,Dt,Dr,v0,m,qB,omega0,beta,eps,
	...
	Deriv deriv(N,L,Dt,Dr,v0,m,qB,omega,beta,eps,sigma,seed);

	// equilibrate: integrate until teq
	integrate(r,dr,v,dv,p,dp,deriv,0,teq,dt);

	int errorCount = 0;
	vector<double> x(navg,0.);
	for( int n =0;n<navg;n++) {
		integrate(r,dr,v,dv,p,dp,deriv,0,tf-dt,dt);
		...
		for(int i=0;i<NpAvg;++i) {
			pAvgx[i][0] += pAvgx_temp[i][0]/(navg*bs_pAvg);
			pAvgx[i][1] += pAvgx_temp[i][1]/(navg*bs_pAvg);
			pAvgx[i][2] += pAvgx_temp[i][2]/(navg*bs_pAvg);
			
			pAvgy[i][0] += pAvgy_temp[i][0]/(navg*bs_pAvg);
			pAvgy[i][1] += pAvgy_temp[i][1]/(navg*bs_pAvg);
			pAvgy[i][2] += pAvgy_temp[i][2]/(navg*bs_pAvg);
			
			pAvgz[i][0] += pAvgz_temp[i][0]/(navg*bs_pAvg);
			pAvgz[i][1] += pAvgz_temp[i][1]/(navg*bs_pAvg);
			pAvgz[i][2] += pAvgz_temp[i][2]/(navg*bs_pAvg);

			rhox[i] += rhox_temp[i]/(navg*N*bs_pAvg);
			rhoy[i] += rhoy_temp[i]/(navg*N*bs_pAvg);
			rhoz[i] += rhoz_temp[i]/(navg*N*bs_pAvg);

			fluxy[i] += fluxy_temp[i]/(navg*L*L);
			fluyx[i] += fluyx_temp[i]/(navg*L*L);
			fluyy[i] += fluyy_temp[i]/(navg*L*L);
